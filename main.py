...
async def on_message_edit(before, after):
    ...
    if after.content.lower().startswith("!evalbot new event"):
        if after.author.id == after.channel.guild.owner_id:
            ...
            embed = discord.Embed(title="PERMISSION DENIED", description="Sorry, you're not authorized to perform this command", color=0xcc0000, timestamp = after.created_at) 
            await after.channel.send(embed=embed)
    ...
    # Delete event from database
        ...
        if after.author.id == after.channel.guild.owner_id:
            ...
            if response["success"]:
                embed = discord.Embed(title="EVENT DELETED", description=response["message"], color=0x34eb71, timestamp = after.created_at)   
            else:
                embed = discord.Embed(title="Something went wrong", description=response["message"], color=0xe60000, timestamp = after.created_at)
            await after.channel.send(embed=embed)
        else:
            embed = discord.Embed(title="PERMISSION DENIED", description="Sorry, you're not authorized to perform this command", color=0xcc0000, timestamp = message.created_at) 
            await after.channel.send(embed=embed)
    ...
    # Export Eligible Participants' list in csv format
        ...
        if after.author.id == after.channel.guild.owner_id:
            ...
            if response["success"]:
                owner = after.author
                await owner.send(file=response["message"])
                embed = discord.Embed(title="CSV FILE GENERATED", description="File has been sent to your DM", color=0x34eb71, timestamp = after.created_at)
                await after.channel.send(embed=embed)
            else:
                embed = discord.Embed(title="Something went wrong", description=response["message"], color=0xe60000, timestamp = after.created_at)
                await after.channel.send(embed=embed)
